...
public class TwoThreeTree<K extends Comparable<K>, V> {
	...
	// TODO: Rewrite comparator for key, stuff should be sorted by occurrence count with the occurrence
	// number to separate two keys. Word is still the key since count needs to be updated.
	// Also, need to write a iterator to get one element at the time. And for density and other
	// functions it would be neat if we had the num of elements stored in a global var, so we don't need
	// to traverse the tree each time to find it.

	private Node root;
	private int nodeCount = 0; // We are only creating new nodes when splitting (with a few exceptions). Just count the splits and
								// we get the size.
	...
	/**
	 * Retrieves the value of the key stored in the tree. If the key is not in the tree a null value is
	 * returned.
	 * 
	...
	public void put (K key, V value) {
		...
		else // If the node is a ThreeNode, then it's more complicated. Create a temporary FourNode, split it to
				// a TwoNode and merge that back into the tree.
		{
	...
	/**
	 * Returns the depth of the entire tree, since it's a balanced tree the depth is the same in the
	 * entire tree.
	 * 
	...
	public int howMuchMore () {
		// Get the number of different nodes for root. Number of TwoNodes is in index 0, ThreeNodes is index
		// 1.
	...
	public float density () {
		// Get the number of TwoNodes and ThreeeNodes in our tree. TwoNodes is stored at index 0, ThreeNodes
		// at 1.
		int [] totalRes = CountNodeTypes(root);

		// Average value of the number of TwoNodes and ThreeNodes. Need to multiply by 2 and 3 because
		// that's their weight.
	 ...
	 * @param returnNullOnMissing If the key is missing in the tree, should return null or the node the
	...
	private Node getNode (K key, Node startNode, boolean returnNullOnMissing) {
		// If we should return null on a missing key then the branch we've just gone into will be null if it
		// is where the key should be.
		if (returnNullOnMissing) {
			if (startNode == null)
				return null;
		}
		else // If we should find the node the key should be in, we need to check for an empty branch before
				// going down, so we can return before that happens.
		{
			// Doesn't matter what branch we check for nulls since we have a balanced tree, if any branch is
			// null we have a leaf node.
	...
	/**
	 * Inserts a FourNode into the tree. If we try to merge with a three node we recursively try to go
	 * up the three until we can insert.
	 * 
	...
	private void put4NodeInTree (Node currentNode, FourNode tmpFourNode) {
		...
		// If we've worked our way up to the root, then we don't need to merge, just set the root to the
		// split result.
		if (currentNode == root) {
			root = splitResult;
			nodeCount++;
	 ...
	 * @return If merged with a ThreeNode we get a resultant FourNode as a result that needs to be
	 *         merged again.
	 */
